  ...
  return (
    <div className="min-h-screen bg-bg pt-36 pb-24 px-8 md:px-20">
      <div className="max-w-screen-xl mx-auto">
        ...
        <h1 className="display-2 text-body mb-4">Releases</h1>
        <p className="body-lg max-w-xl mb-14">Each release has its own page with streaming links when you add them in the studio.</p>
            ...
            <li key={r.slug}>
              <Link href={`/releases/${r.slug}`} className="group block">
                <div className="manuscript-frame relative aspect-square overflow-hidden mb-4 shadow-[0_4px_24px_rgba(13,11,8,0.06)]">
                  ...
                  />
                  {r.isNew && (
                    <span className="absolute top-3 left-3 ui-label px-2 py-1 text-[10px] bg-accent text-ivory">New</span>
                  )}
                </div>
                <p className="font-playfair text-base text-body group-hover:text-accent transition-colors">{r.title}</p>
