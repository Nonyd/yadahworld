...
export default function SiteEventForm({ mode, initial }: { mode: Mode; initial?: SiteEvent }) {
    ...
    try {
      if (mode === 'create') {
        ...
        if (!res.ok) {
          ...
          throw new Error((j as { error?: string }).error || 'Failed to create')
        }
        router.push('/admin/homepage-events')
        router.refresh()
        return
      }
      if (!initial?.id) return
      const res = await fetch(`/api/admin/site-events/${initial.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      })
