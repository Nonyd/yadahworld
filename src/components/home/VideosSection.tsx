...
export default function VideosSection({ videos, copy }: { videos: PublicVideo[]; copy: SiteCopy }) {
  ...
  const { openAtVideoIndex, videoLightbox } = usePublicVideoLightbox(display)
          ...
          {display.map((video, i) => (
            ...
            >
              <button
                ...
                className="group relative aspect-video w-full cursor-pointer overflow-hidden border border-[rgba(42,37,32,0.08)] text-left manuscript-frame shadow-[0_4px_24px_rgba(13,11,8,0.08)] focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
              >
                <Image
                  src={video.thumbnail}
                  alt=""
